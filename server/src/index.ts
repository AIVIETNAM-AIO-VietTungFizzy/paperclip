...
export async function startServer(): Promise<StartedServer> {
  ...
  if (config.deploymentMode === "local_trusted" && config.deploymentExposure === "private" && !isLoopbackHost(config.host)) {
    console.log(
      `[paperclip] local_trusted mode with private exposure binding to ${config.host} (non-loopback). ` +
      "This is allowed for private Docker deployments.",
    );
  } else if (config.deploymentMode === "local_trusted" && !isLoopbackHost(config.host)) {
    throw new Error(
      `local_trusted mode requires loopback host binding (received: ${config.host}). ` +
      "Use authenticated mode or set exposure to private for non-loopback deployments.",
    );
  }
